...
export default {
  ...
  methods: {
    ...
    // 加入购物车点击事件,加入购物车的时候需要发动请求
    async addShopCart() {
      //⭐🍎   dispatch传递多个参数要用对象形式
      // 这里需要判断请求是否成功，然后再进行相应的操作
      //  在action中，让下面的调用返回值是一个promise对象    ⭐要想接收promise就得有async/await
      // try  catch失败回调    判断成功还是失败(这里要用asyns和await的组合)
      try {
        await this.$store.dispatch('addShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
        //路由跳转   需要传递的参数：产品的个数...      ⭐须转化为字符串
